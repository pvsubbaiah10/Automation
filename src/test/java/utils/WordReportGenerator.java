...
public class WordReportGenerator {
    ...
    public static void init(String featureName) {
        if (!docMap.containsKey(featureName)) {
            ...
            String timestamp = new SimpleDateFormat("hh-mm-ssa").format(now);
    ...
    public static void addStep(String featureName, String stepName, String status, String screenshotPath) {
        ...
        if (screenshotPath != null && Files.exists(Paths.get(screenshotPath))) {
            try (InputStream pic = new FileInputStream(screenshotPath)) {
                ...
                imgRun.addBreak();
                imgRun.addPicture(pic, XWPFDocument.PICTURE_TYPE_PNG, screenshotPath, Units.toEMU(450), Units.toEMU(300));
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    public static void addSummary(String featureName) {
        XWPFDocument doc = docMap.get(featureName);
        if (doc == null) return;

        int passed = passCount.getOrDefault(featureName, 0);
        int failed = failCount.getOrDefault(featureName, 0);
        ...
        run.setText("\nExecution Summary:");
        run.addBreak();
        run.setText("Total Steps: " + total);
        run.addBreak();
        run.setText("Passed: " + passed);
        run.addBreak();
        run.setText("Failed: " + failed);
    }
